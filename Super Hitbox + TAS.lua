...
-- TODO use luacanvas to draw blocks once: 1/ use a cache to save the drawing for a room  2/ save PLMs in a table to draw them quickly
-- TODO add grapcalc
...
local function displaySamusHitbox(cameraX, cameraY, samusXPosition, samusYPosition)
    ...
    local samusXPosition32 = samusXPosition * 0x10000 + samusXSubposition
    local previousSamusXPosition32 = previousSamusXPosition * 0x10000 + previousSamusXSubposition
    local samusYPosition32 = samusYPosition * 0x10000 + samusYSubposition
    local previousSamusYPosition32 = previousSamusYPosition * 0x10000 + previousSamusYSubposition
    local samusXSpeed32 = samusXSpeed * 0x10000 + samusXSubspeed
    local samusXMomentum32 = samusXMomentum * 0x10000 + samusXSubmomentum

    local xDistance_expected = samusXSpeed32 + samusXMomentum32
    local xDistance_actual = math.abs(samusXPosition32 - previousSamusXPosition32)
    local yDistance = math.abs(samusYPosition32 - previousSamusYPosition32)

    local colour = colour_samus
    if xDistance_actual > xDistance_expected then
        colour = colour_samus_armPumping
    elseif xDistance_actual == xDistance_expected then
        colour = colour_samus
    elseif xDistance_actual > xDistance_expected * 3 / 4 then
        colour = colour_samus_2
    elseif xDistance_actual > xDistance_expected / 2 then
        colour = colour_samus_3
    else
        colour = colour_samus_4
    end
    ...
    if samusXPosition32 ~= previousSamusXPosition32 then
        ...
        gui.text(infohudCol1, infohudRow2,
            string.format("dx: %s%4d.%05d", direction, (xDistance_actual >> 0x10), (xDistance_actual & 0xFFFF)))
    end

    -- Show Y distance
    if samusYPosition32 ~= previousSamusYPosition32 then
        local direction = 'v'
        if samusYPosition32 < previousSamusYPosition32 then
            direction = '^'
        end

        gui.text(infohudCol1, infohudRow3,
            string.format("dy: %s%4d.%05d", direction, (yDistance >> 0x10), (yDistance & 0xFFFF)))
    end
...
local function displayItemPercentage()
    ...
    local percent = 0
    percent = percent + math.floor(sm.getSamusMaxHealth() / 100)
    percent = percent + math.floor(sm.getSamusMaxReserveHealth() / 100)
...
local function displayActiveGlitches(samusXPosition, samusYPosition)
    function slopeKiller()
        ...
        isRelevantMovementType = isRelevantMovementType or movementType == 5    -- crouching
    ...
    activeGlitches = activeGlitches .. flashSuit()
    activeGlitches = activeGlitches .. xMode()
...
local function displayActiveTricks(samusXPosition, samusYPosition)
    function btSkip()
        ...
        isActive = isActive and samusXPosition == 0xB4 and sm.getSamusXSubposition() == 0xFFFF
        isActive = isActive and samusYPosition == 0xBC and sm.getSamusYSubposition() == 0x93FF
        isActive = isActive and sm.getSamusXMomentum() == 1 and 0 < samusXSubmomentum and samusXSubmomentum < 0x4000
...
local function on_paint()
    ...
    previousSamusXSubposition = sm.getSamusXSubposition()
